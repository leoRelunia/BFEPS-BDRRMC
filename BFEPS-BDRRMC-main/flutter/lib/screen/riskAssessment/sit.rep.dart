...
class _SitRepPageState extends State<SitRepPage> {
  ...
  Future<void> _fetchRecords() async {
    try {
      final url = Uri.parse('http://localhost/BFEPS-BDRRMC-main/BFEPS-BDRRMC-main/api/riskassessment/sitrep/get_report.php'); 
  ...
  void _deleteReport(String id) async {
    ValidationDialog.showDeleteDialog(
      ...
      onConfirm: () async {
        ...
        try {
          final url = Uri.parse('http://localhost/BFEPS-BDRRMC-main/BFEPS-BDRRMC-main/api/riskassessment/sitrep/delete_report.php');
  ...
  List<Map<String, dynamic>> _getPaginatedRecords() {
    ...
    List<Map<String, dynamic>> filteredRecords = _allRecords.where((report) {
      ...
      String fullName = "${report['calamity_name'] ?? ''}"
          .replaceAll(',', '')
          .trim()
          .toLowerCase();
      ...
      return Container(
        ...
        child: Row(
          children: [
            ...
            DataCellWidget(value: report['calamity_name'] ?? '', flex: 2, center: true),
            Expanded(
  flex: 6,
  child: Row(
...
Widget build(BuildContext context) {
  return Scaffold(
    appBar: AppBar(
      title: Text(
        "${widget.report['calamity_name']} - Situational Report",
              ...
              ElevatedButton.icon(
                onPressed: () {
                  _createPdf(PdfPageFormat.a4, widget.report).then((bytes) {
                    Printing.sharePdf(
                      bytes: bytes,
                      filename: "${widget.report['calamity_name']}_report.pdf",
...
class _SitRepFormState extends State<SitRepForm> {
  ...
  void initState() {
    ...
    if (widget.existingData != null) {
      ...
      calamityController.text = widget.existingData!['calamity_name'] ?? '';
      ...
      recommendationsController.text = widget.existingData!['recommendations_needs'] ?? '';
    }
  }
 ...
 child: SingleChildScrollView(
  ...
  Future<void> _saveData(BuildContext context) async {
    if (calamityController.text.isNotEmpty &&
      ...
      try {
        final data = { 
          'calamity_name': toCamelCase(calamityController.text),
          ...
          'recommendations_needs': recommendationsController.text,
        };

        var url = widget.isUpdateMode 
            ? Uri.parse('http://localhost/BFEPS-BDRRMC-main/BFEPS-BDRRMC-main/api/riskassessment/sitrep/update_report.php') 
            : Uri.parse('http://localhost/BFEPS-BDRRMC-main/BFEPS-BDRRMC-main/api/riskassessment/sitrep/save_report.php');
