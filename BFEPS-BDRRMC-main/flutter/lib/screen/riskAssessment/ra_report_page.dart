...
class RaReportPageState extends State<RaReportPage> {
  ...
  Future<void> _fetchRecords() async {
    try {
      final url = Uri.parse('http://localhost/BFEPS-BDRRMC-main/BFEPS-BDRRMC-main/api/riskassessment/riskassessmentreport/get_report.php'); 
  ...
  void _deleteReport(String id) async {
    ValidationDialog.showDeleteDialog(
      ...
      onConfirm: () async {
        ...
        try {
          final url = Uri.parse('http://localhost/BFEPS-BDRRMC-main/BFEPS-BDRRMC-main/api/riskassessment/riskassessmentreport/delete_report.php');
  ...
  List<Map<String, dynamic>> _getPaginatedRecords() {
    ...
    List<Map<String, dynamic>> filteredRecords = _allRecords.where((report) {
      ...
      String fullName = "${report['household_head_name'] ?? ''}"
          .replaceAll(',', '')
          .trim()
          .toLowerCase();

      String zoneNum = "${report['household_zone'] ?? ''}"
          .replaceAll(',', '')
          .trim()
          .toLowerCase();
  ...
  void _goToNextPage() {
    int totalFilteredRecords = _allRecords
        .where((report) =>
            ...
            report['household_head_name'].toString().toLowerCase().contains(_searchQuery.toLowerCase()) ||
            report['household_zone'].toString().toLowerCase().contains(_searchQuery.toLowerCase()))
        .length;
      ...
      return Container(
        ...
        child: Row(
          children: [ 
            ...
            DataCellWidget(value: report['household_zone'] ?? '', flex: 1),
            DataCellWidget(value: report['household_head_name'] ?? '', flex: 1),
            DataCellWidget(value: report['risk_type'] ?? '', flex: 1),
            DataCellWidget(value: report['num_of_pwd'] ?? '', flex: 1, center: true),
            DataCellWidget(value: report['num_of_senior'] ?? '', flex: 1, center: true),
            DataCellWidget(value: report['num_of_infant_toddler'] ?? '', flex: 1, center: true),
            DataCellWidget(value: report['num_of_flood_fatality'] ?? '', flex: 1, center: true),
            DataCellWidget(value: report['num_of_property_damage'] ?? '', flex: 1, center: true),
...
class _ReliefReportFormState extends State<RAReportForm> {
  ...
  void initState() {
    ...
    if (widget.existingData != null) {
      ...
      householdNameController.text = widget.existingData!['household_head_name'] ?? '';
      ...
      selectedZone = widget.existingData!['household_zone'];
  ...
  if (householdNameController.text.isNotEmpty &&
    ...
    try {
      final data = {
        ...
        'household_head_name': toCamelCase(householdNameController.text),
      ...
      var url = widget.isUpdateMode
          ? Uri.parse('http://localhost/BFEPS-BDRRMC-main/BFEPS-BDRRMC-main/api/riskassessment/riskassessmentreport/update_report.php')
          : Uri.parse('http://localhost/BFEPS-BDRRMC-main/BFEPS-BDRRMC-main/api/riskassessment/riskassessmentreport/save_report.php');
