...
import 'package:bfeps_bdrrmc/screen/calamity/evacuation_centers.dart';
import 'package:bfeps_bdrrmc/screen/riskAssessment/flood_updates.dart';
import 'package:bfeps_bdrrmc/screen/riskAssessment/ra_report_page.dart';
...
class CalamityPage extends StatefulWidget {
  const CalamityPage({super.key, required void Function(String p1, Widget p2) onMenuSelect});
...
class CalamityPageState extends State<CalamityPage> {
  ...
  Future<void> _fetchRecords() async {
    try {
      final url = Uri.parse('http://localhost/BFEPS-BDRRMC-main/BFEPS-BDRRMC-main/api/evacuation/get_calamity.php'); 
  ...
  void _deleteCalamity(String id) async {
    ValidationDialog.showDeleteDialog(
      ...
      onConfirm: () async {
        ...
        try {
          final url = Uri.parse('http://localhost/BFEPS-BDRRMC-main/BFEPS-BDRRMC-main/api/evacuation/delete_calamity.php');
...
Widget _buildTableRows() {
  ...
  return ListView.builder(
    ...
    itemBuilder: (context, index) {
      ...
      bool isUnderControl = calamity['current_status'] == 'Under Control'; // Check if resolved
              ...
              child: Center(
                child: PopupMenuButton<String>(
                  onSelected: (value) {
                    ...
                    else if (value == 'RA Report' && !isOngoing && !isResolved) {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                    ...
                    } else if (value == 'Flood Updates' && !isResolved && !isUnderControl) {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                  ...
                  itemBuilder: (context) => [
                    ...
                    if (!isResolved && !isUnderControl) const PopupMenuItem(value: 'Flood Updates', child: Text('View Flood Updates')),
                    if (!isOngoing && !isResolved) const PopupMenuItem(value: 'RA Report', child: Text('View RA Report')),
                    if (isResolved || !isResolved) // Ensures Evacuation Center always appears
                      const PopupMenuItem(
                        value: 'Evacuation Center',
                        child: Text('View Evacuation Center'),
                      ),
                  ],
                ),
              ),
    ...
    if (dateController.text.isNotEmpty &&
      ...
      try {
        ...
        var url = widget.isUpdateMode 
            ? Uri.parse('http://localhost/BFEPS-BDRRMC-main/BFEPS-BDRRMC-main/api/evacuation/update_calamity.php') 
            : Uri.parse('http://localhost/BFEPS-BDRRMC-main/BFEPS-BDRRMC-main/api/evacuation/save_calamity.php');
